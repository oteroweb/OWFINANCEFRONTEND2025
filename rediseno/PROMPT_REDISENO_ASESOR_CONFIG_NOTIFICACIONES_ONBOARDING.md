...
✅ **Actualizado 2026-07-25 — diseño real completado sobre una base ya sólida (no greenfield, a diferencia de Admin).**
- **Asesor IA** (`AIAdvisorPanel.jsx`): ya tenía header/avatar/en línea/quick replies/CTAs/typing indicator — se completó con lo que faltaba del prompt: bottom-sheet de ajustes (nombre/personalidad 3 chips/toggle activado), barra de error real con Reintentar/OK (antes el error se mostraba como una burbuja falsa del asistente), botón copiar por mensaje (hover), botón limpiar conversación.
- **Configuración**: `ConfigRoute.jsx` (lista simple) se usaba para Pro Y Lite con solo un card extra de tasas — violación de DESIGN_CONTRACT.md §5, mismo patrón que Cántaros antes del fix de OWF-322. Nuevo `ProConfigRoute.jsx`: cards + 5 tabs (Perfil con avatar+`ChangePasswordCard`, Finanzas con banners condicionales, Categorías/Cuentas/Impuestos como **listas funcionales simples** — el árbol completo con drag&drop es alcance del módulo separado Cuentas/Categorías/Impuestos, todavía sin su propio ciclo de diseño real, no duplicado acá).
- **Notificaciones**: solo existía el panel (`NotificationsPanel.jsx`, ya sólido). Nuevo `NotificationsRoute.jsx` — página completa `/user/notifications`, con su propio estado "leído" independiente del panel (mismo comportamiento documentado en §3.3, no un bug). El botón "Ver todas las notificaciones" del panel no tenía `onClick` real — ahora navega a la página nueva.
- **Onboarding**: `OnboardingFlow.jsx` ya cubría el wizard de 6 pasos completo (welcome/about/situation/goals/jars/done) — se agregaron los badges de gamificación (🌱 Semilla <40% / 🌿 Brote 40-74% / 🌳 Árbol ≥75%) en el paso final, que faltaban por completo (solo había un checkmark genérico).

⚠️→✅ **Actualizado 2026-07-25 — el gap de `OnboardingModal` (§4.1) ya está resuelto, ver `PROMPT_REDISENO_AUTH_PUBLICO.md`.** En esta pasada, la elección Lite/Pro vivía fusionada dentro del paso "welcome" del wizard de arriba (no la que describe la §4.1: pantalla completa independiente, no saltable, antes del wizard). Se construyó el `OnboardingModal` real y se sacó la elección del wizard — ahora son 2 flujos genuinamente independientes, como especifica este documento.
- **Verificado en vivo en el kit** (sin errores de consola): panel de Asesor IA con ajustes/error/copiar, las 5 tabs de Config Pro, navegación panel→página de Notificaciones. **El badge de gamificación del Onboarding no se pudo re-verificar visualmente en este entorno local** — el panel de preview sirvió una versión repetida del script pese a que el código en disco (confirmado con `Read`/`grep`) y el bundle subido a Claude Design son correctos; recomendado verificarlo directamente en Claude Design.
