...
⚠️ **Actualizado 2026-07-23 — el lado Pro de este documento ya no es un "spec para construir desde cero".** El usuario portó su propio diseño de Claude Design a Vue directamente (**OWF-322**, 2026-07-20): KPI bar, resumen del mes, tabla completa, configuración global y editor con drag&drop ya están implementados función por función en `jars/index.vue` + `MonthlyIncomePanel.vue` + `JarSetting`/`JarLeverageSetting` (backend). Además, en paralelo (misma fecha), se reconectó el lado del *diseño* (`ui_kits/lite-desktop/organisms/JarsProConfig.jsx` + `JarsProEditor.jsx` + `templates/ProJarsRoute.jsx`) que existía huérfano — ver `PROMPT_REDISENO_CENTRAL.md` para el detalle de ese fix de wiring. **Este documento sigue siendo útil como referencia de paridad/QA** (para verificar que nada quedó fuera al comparar diseño↔Vue), pero ya no es la fuente de "qué falta construir" para Pro.

✅ **Actualizado 2026-07-25 — Lite (sección 2) ya tiene diseño real**, no solo el placeholder mínimo descrito abajo. `ui_kits/lite-desktop/organisms/JarsLiteConfig.jsx` (nuevo) + `templates/JarsRoute.jsx` implementan card resumen (total + barra de distribución + 3 stats), selector de periodo (chips, cosmético — sigue sin recargar datos, ver §3), lista de cántaros con drag reorder visual + toggle activo inline + tags Excedido/Lleno/Inactivo, y 3 modales: Detalle (hero + stats 2x2 + acciones rápidas Agregar/Retirar + Eliminar/Editar), Editar (nombre + % + paleta de 8 colores) y Nuevo cántaro. Verificado end-to-end en el kit (crear cántaro, editar, toggle activo, todas las transiciones de modal) sin errores de consola. **Diferencia deliberada de adaptación**: en vez de bottom-sheets (patrón mobile real de `LiteJarsView.vue`/Quasar), se usó el mismo modal centrado que ya establece `JarsProEditor.jsx` (`JarAdjustModal`) — es el lenguaje visual ya vigente en este kit de escritorio, no una regresión. Este documento sigue sirviendo como referencia de paridad — el `JarsLiteConfig.jsx` cubre la sección 2 completa salvo drag&drop de categorías/apalancamiento/meta (que la §3 marca explícitamente como exclusivos de Pro, no aplica a Lite).
