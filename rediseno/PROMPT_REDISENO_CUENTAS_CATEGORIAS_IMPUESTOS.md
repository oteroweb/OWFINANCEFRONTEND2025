...
✅ **Actualizado 2026-07-25 — diseño real completado, greenfield (no había plantillas de ruta ni árboles interactivos previos, solo organismos de apoyo).** Nuevo `organisms/AccountsCategoriesTaxes.jsx`: `CategoriesTreeView` + `CategoryDialog` (unificado — se resolvió la inconsistencia §2.1 vs §2.3 a favor del diálogo completo: icono + tipo de transacción + incluir en balance), `AccountsTreeView` + `AccountDialog` + `AdjustBalanceDialog` (con "Recalcular saldo" **real** con loading, no el simulado de `AccountFilterWidget.jsx` — decisión §9 resuelta), y `TaxesCrud` (reemplaza el placeholder `ConfigTaxesTab` de `ProConfigRoute.jsx` — ahora **con confirmación antes de eliminar**, unificando hacia "más seguro" en las 3 piezas, no solo Categorías). Integrado directamente en las tabs Categorías/Cuentas/Impuestos de `ProConfigRoute.jsx` (ya existían como placeholders desde el módulo de Config, ahora con el componente real). Verificado en el kit: árbol de categorías con edición/catálogo de iconos, cuentas con ajustar/recalcular saldo, impuestos con confirmación de borrado real. **Bug real encontrado y corregido en el camino**: el `CategoryDialog` pasaba `options` a `Segmented` en formato de tupla (`[id, label]`) en vez del formato de objeto (`{value, label}`) que el componente compartido realmente espera — el click no rompía visualmente pero `onChange` siempre recibía `undefined`, o sea el selector de Tipo de transacción no funcionaba. Corregido. **No se pudo re-verificar visualmente el fix en este entorno local** (mismo patrón de staleness del panel de preview ya documentado en Cántaros/Onboarding — el código en disco es correcto, confirmado con `grep`); recomendado confirmar en Claude Design.
