...
const SETTINGS_GROUPS = [
  {
    label: 'Cuenta',
    items: [
      { icon: 'person',        label: 'Perfil',             hint: 'José Otero', nav: 'profile' },
      { icon: 'insights',      label: 'Mi perfil financiero', hint: 'Ingresos, cántaros, metas y asesor IA', nav: 'finprofile' },
...
function SettingsScreen({ onBack, mode = 'lite', onModeChange, theme = 'dark', onThemeChange, onGoTo, onStartOnboarding }) {
  const initToggles = {};
              ...
              {group.items.map((item, ii) => (
                <div key={ii}>
                  ...
                  >
                    ...
                    {(item.chevron || item.nav || item.act) && !item.toggle && <span className="material-icons" style={{ fontSize: 18, color: 'var(--fg-3)' }}>chevron_right</span>}
                  </button>
                </div>
              ))}
            </div>
