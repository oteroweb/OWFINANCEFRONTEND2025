...
| 7 | **Configuración** | `screens/SettingsScreen.jsx` | Grouped settings (Cuenta · Visualización · Notificaciones) with toggles + cerrar sesión. **Cuenta** ahora abre *Perfil*, *Mi perfil financiero* y *Repetir configuración inicial* (onboarding) | Bottom nav (Settings tab) |
...
| 9 | **Perfil** ⭐ | `screens/ProfileScreen.jsx` | Identidad (avatar + medidor de completitud) · Datos personales · Contacto · Ubicación. Atajo a *Mi perfil financiero* | Configuración → *Perfil* |
| 10 | **Mi perfil financiero** ⭐ | `screens/FinancialProfileScreen.jsx` | Perfilado para el **asesor IA**: 4 cards de chips (Quién soy · Situación financiera · Metas y sueños · Mis cántaros) + selector de plantilla de cántaros + tabla editable con propósito | Configuración → *Mi perfil financiero* |
| 11 | **Onboarding** ⭐ | `components/OnboardingMobile.jsx` | Wizard post-creación de cuenta (overlay full-screen): bienvenida (Lite/Pro) → quién eres → situación → metas → cántaros → listo. Pasos 1–4 con *Saltar* | Configuración → *Repetir configuración inicial* · `window.__owStartOnboardingMobile()` |
...
ui_kits/mobile/
  ...
  components/
    ...
    ProfileComponents.jsx ⭐ MChipGroup · MJarMiniBar · MJarTemplateSelector · MJarsTable (perfilado IA)
    OnboardingMobile.jsx  ⭐ OnboardingMobile — wizard post-creación de cuenta (overlay)

  profile-data.jsx        ⭐ PROFILE_FIELDS (chips) · JAR_TEMPLATES · DREAM_PLACEHOLDERS · AI_PROFILE · USER_JARS
  ...
  screens/
    ...
    ProfileScreen.jsx ⭐ · FinancialProfileScreen.jsx ⭐
```

> **Onboarding ↔ Perfil financiero comparten fuente.** Ambos leen `profile-data.jsx` y los mismos componentes de chips/cántaros (`ProfileComponents.jsx`), así que editar un campo del perfilado se refleja en los dos. El perfil alimenta al **asesor IA** (campo `onboarding_profile_completed` + chips). Fuente del diseño: `docs/00-sistema/DESIGN_PROMPT_ONBOARDING.md`.
