...
✅ **Actualizado 2026-07-25 (parte 1) — Autenticación completada.** `organisms/EntryGate.jsx` (landing + login/registro combinados) ya cubría bien el flujo base — se completó con lo que documentaba este prompt como faltante: **medidor de fuerza de contraseña** (mismo criterio de 4 segmentos que `ChangePasswordCard`, en registro y en el nuevo reset), **enlace "¿Olvidaste tu contraseña?"** (solo en login), **botones sociales decorativos** (Google/Apple, sin función real, igual que en `LoginPage.vue`), y — el gap real más grande — un **flujo completo de recuperación de contraseña** que no existía en absoluto: `GateForgotPassword` (email → banner de éxito) y `GateResetPassword` (nueva contraseña + confirmar + medidor de fuerza → banner de éxito), matching `ForgotPasswordPage.vue`/`ResetPasswordPage.vue`. Verificado end-to-end: login→olvidé mi contraseña→enviar enlace→banner→ya tengo el enlace→nueva contraseña con medidor "Fuerte"→guardar→banner de éxito.

✅ **Actualizado 2026-07-25 (parte 2) — Marketing completado + inconsistencia §4 resuelta.** Nuevo `organisms/MarketingPages.jsx`: `FeaturesPage` (nav de anclas a las 5 secciones + Lite vs Pro), `PricingPage` (toggle mensual/anual, 3 planes, FAQ acordeón, **y la nota de producto del hallazgo §3 explícita en la propia página** — "hoy no existe paywall técnico, confirmar con producto antes de lanzar"), `MatrixPage` (2 tarjetas de filosofía + tabla completa por sección). **Inconsistencia FeaturesPage↔MatrixPage (§4) resuelta con datos reales, no con ninguna de las 2 fuentes contradictorias**: "Transferir entre cántaros" no es una feature real con ese nombre — el mecanismo real es **Apalancamiento**, 100% exclusivo de Pro (ninguna fuente tenía razón: ni "parcial en Lite" ni "completo en ambos"); "Carga masiva" está disponible **igual en Lite y Pro**, no "simplificada en Lite" como decía la matriz vieja. Ambas páginas ahora comparten un único array `LITE_PRO_MATRIX` en vez de repetir (y volver a divergir) la tabla. `GateLanding` se expandió con las secciones que le faltaban del prompt §2.1: trust strip, comparación Lite/Pro compacta (con link a la matriz completa), features grid, CTA final — y su topbar ahora usa `MarketingNav` (mismo componente que las 3 páginas nuevas) para navegación consistente entre Inicio/Funciones/Planes/Comparativa. Verificado end-to-end: navegación completa por las 4 páginas, toggle anual, acordeón FAQ, sin errores de consola.

✅ **Además (fuera del alcance original de este documento, pedido explícito del usuario) — se completó el Onboarding real.** La elección Lite/Pro vivía fusionada dentro del wizard `OnboardingFlow.jsx` (decisión de una sesión anterior) en vez de ser el flujo `OnboardingModal` independiente que describe `PROMPT_REDISENO_ASESOR_CONFIG_NOTIFICACIONES_ONBOARDING.md` §4.1 (pantalla completa, no saltable, dispara ANTES del wizard de perfil). Se construyó `OnboardingModal` real (2 tarjetas grandes Lite/Pro, fondo azul de marca, sin botón de cerrar) y se sacó la elección del paso "welcome" del wizard — ahora son 2 flujos genuinamente independientes, como en producción. Verificado end-to-end: registro→`OnboardingModal`→"Elegir Pro"→modo seteado a Pro (confirmado con `window.getMode()`)→`OnboardingFlow` se abre automático después, sin el picker duplicado.
