...
const CONFIG_GROUPS = [
  {
    label: 'Cuenta',
    items: [
      { icon: 'person',       label: 'Perfil',              hint: 'José Otero · jose@owfinance.com' },
      { icon: 'savings',      label: 'Cuentas vinculadas',  hint: '3 tarjetas · 2 bancos' },
      { icon: 'receipt_long', label: 'Exportar datos',      hint: 'CSV, PDF' },
    ],
  },
  {
    label: 'Visualización',
...
function ConfigRoute({ rates = {}, onRatesChange }) {
  const initToggles = {};
...
function ConfigRow({ item, first, value, onToggle }) {
  return (
    <div
      onClick={onToggle}
      style={{
    ...
    >
      ...
      {item.chevron && !item.toggle && <span className="material-icons" style={{ fontSize: 18, color: 'var(--fg-3)' }}>chevron_right</span>}
    </div>
  );
}
