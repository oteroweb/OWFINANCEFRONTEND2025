...
function MobileShell({ mode: initialMode = 'lite' }) {
  ...
  useShellMobileEffect(() => {
    ...
    return () => { if (window.__owOpenTxDetail) delete window.__owOpenTxDetail; };
  }, []);
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden', position: 'relative', background: 'var(--bg-canvas)' }}>
      ...
      <div style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column', position: 'relative' }}>
        ...
        {route === 'config'       && <SettingsScreen mode={mode} onModeChange={setMode} theme={theme} onThemeChange={setTheme} onBack={() => goTo('home')} />}
